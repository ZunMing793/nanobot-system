...
def _load_models_config(config: Config) -> dict:
    """Load models.json configuration."""
    models_path = config.shared_skills_path
    if models_path:
...
def _make_provider_for_model(config: Config, selected_model: str, models_config: dict | None = None):
    ...
    if models_config:
        ...
        if model_cfg:
            ...
            provider_cfg = models_config.get("providers", {}).get(provider_name_from_cfg, {})
...
def gateway(
    ...
    agent = AgentLoop(
        ...
        models_config_path=config.shared_skills_path.parent / "config" / "models.json" if config.shared_skills_path else None,
