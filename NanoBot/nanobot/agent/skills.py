...
class SkillsLoader:
    ...
    In multi-bot system, skills are loaded from:
    1. shared_skills_path - shared skills directory
    2. builtin_skills_path - built-in skills from NanoBot/nanobot/skills
    """
    ...
    def __init__(
        ...
        self.shared_skills = shared_skills_path
        self.builtin_skills = builtin_skills_path

    def list_skills(self, filter_unavailable: bool = True) -> list[dict[str, str]]:
        """
        List all available skills from shared and builtin directories.
        ...
        if self.shared_skills and self.shared_skills.exists():
        ...
        if self.builtin_skills and self.builtin_skills.exists():
            for skill_dir in self.builtin_skills.iterdir():
                if skill_dir.is_dir():
                    skill_file = skill_dir / "SKILL.md"
                    if skill_file.exists():
                        # Don't duplicate if already in shared
                        if not any(s["name"] == skill_dir.name for s in skills):
                            skills.append({"name": skill_dir.name, "path": str(skill_file), "source": "builtin"})

        if filter_unavailable:
            return [s for s in skills if self._check_requirements(self._get_skill_meta(s["name"], s["source"]))]
        return skills

    def load_skill(self, name: str, source: str = "shared") -> str | None:
        """
        Load a skill by name.

        Args:
            name: Skill name (directory name).
            source: Skill source ('shared' or 'builtin').
    ...
    def load_skill_guide(self, name: str) -> str | None:
        """
        Load the detailed GUIDE.md for a skill (on-demand loading).

        Args:
            name: Skill name (directory name).

        Returns:
            GUIDE.md content or None if not found.
        """
        # Try shared first
        if self.shared_skills:
    ...
    def get_skill_path(self, name: str) -> Path | None:
        ...
        # Try shared first
        if self.shared_skills:
            skill_dir = self.shared_skills / name
            if skill_dir.exists():
                return skill_dir
    ...
    def _get_skill_meta(self, name: str, source: str = "shared") -> dict:
