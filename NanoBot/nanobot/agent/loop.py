...
class AgentLoop:
    ...
    def __init__(
        ...
        self.memory_learning = MemoryLearningManager(
            workspace=workspace,
            shared_learnings_path=shared_learnings_path,
            shared_memory_path=shared_memory_path,
            bot_id=self.bot_id,
    ...
    async def _switch_model(self, provider_name: str, model_name: str) -> None:
        ...
        try:
            ...
            provider_cfg = models_config.get("providers", {}).get(provider_name, {})
            if not provider_cfg:
            ...
            if api_format == "anthropic":
                ...
                new_provider = CustomProvider(
                    api_key=provider_cfg.get("api_key", "no-key"),
                    api_base=provider_cfg.get("api_base", "http://localhost:8000/v1"),
                    default_model=model_name,
                )
            ...
            logger.info(f"Model switched to {provider_name}/{model_name}")
        ...
        if self.command_handler.is_command(msg.content):
            ...
            if command_response:
                ...
                if "[MODEL_SWITCH]" in command_response:
                    import re
                    match = re.search(r"\[MODEL_SWITCH\](\w+)\|([^\n]+)", command_response)
                    if match:
                        provider_name = match.group(1)
                        model_name = match.group(2).strip()
                        await self._switch_model(provider_name, model_name)
                        # Remove the signal from response
                        command_response = re.sub(r"\n?\[MODEL_SWITCH\].+", "", command_response)
    ...
    def _save_turn(self, session: Session, messages: list[dict], skip: int) -> None:
        ...
        from nanobot.utils.timezone import now, now_iso
